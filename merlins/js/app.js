(() => {
    ...
    da.init();
    const logoItem = document.querySelector(".first-block__logo-item");
    document.addEventListener("mousemove", (event => {
        const {clientX, clientY} = event;
        const {left, top, width, height} = logoItem.getBoundingClientRect();
        const xOffset = -(clientX - (left + width / 2)) / width * 15;
        const yOffset = -(clientY - (top + height / 2)) / height * 15;
        logoItem.querySelector("::after");
        logoItem.style.setProperty("--shadow-x", `${xOffset}px`);
        logoItem.style.setProperty("--shadow-y", `${yOffset}px`);
    }));
