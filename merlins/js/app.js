(() => {
    ...
    da.init();
    function initializeShadowEffect(selector) {
        const element = document.querySelector(selector);
        if (!element) return;
        document.addEventListener("mousemove", (event => {
            const {clientX, clientY} = event;
            const {left, top, width, height} = element.getBoundingClientRect();
            const xOffset = -(clientX - (left + width / 2)) / width * 20;
            const yOffset = -(clientY - (top + height / 2)) / height * 20;
            element.style.setProperty("--shadow-x", `${xOffset}px`);
            element.style.setProperty("--shadow-y", `${yOffset}px`);
        }));
    }
    initializeShadowEffect(".first-block__logo-item");
    const featuresEl = document.querySelector(".list-categories");
    const featureEls = document.querySelectorAll(".list-categories__item");
    ...
    const categoryButtons = document.querySelectorAll(".list-categories__button");
    const categoriesBody = document.querySelector(".categories__body");
    const mediaItems = document.querySelectorAll(".media-categories__item");
    categoryButtons.forEach((button => {
        button.addEventListener("click", (() => {
            categoriesBody.classList.add("active");
            mediaItems.forEach((item => item.classList.remove("active-item")));
            const parentClass = button.closest(".list-categories__item").classList[1];
            const targetItem = document.querySelector(`.media-categories__item.${parentClass}`);
            if (targetItem) targetItem.classList.add("active-item");
            categoryButtons.forEach((btn => btn.classList.remove("active-category")));
            button.classList.add("active-category");
        }));
    }));
    const links = document.querySelectorAll(".social-block__link");
    links.forEach((link => {
        const parentItem = link.closest(".social-block__item");
        if (parentItem) {
            const addActiveClass = () => {
                parentItem.classList.add("active-circle");
            };
            const removeActiveClass = () => {
                parentItem.classList.remove("active-circle");
            };
            ...
            link.addEventListener("touchend", removeActiveClass);
        }
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const buttons = document.querySelectorAll(".list-category-page__button");
        buttons.forEach((button => {
            button.addEventListener("click", (() => {
                const popupId = button.getAttribute("data-popup");
                const popup = document.getElementById(popupId);
                if (popup) popup.classList.add("popup-active"); else console.error(`Popup with ID '${popupId}' not found.`);
            }));
        }));
        document.querySelectorAll(".categories-popups__item").forEach((popup => {
            const closeButton = popup.querySelector(".categories-popups__close");
            if (closeButton) closeButton.addEventListener("click", (() => {
                popup.classList.remove("popup-active");
            }));
            popup.addEventListener("click", (event => {
                if (event.target === popup) popup.classList.remove("popup-active");
            }));
        }));
    }));
    function bubbles() {
        document.querySelectorAll(".lemonade-category__title").forEach((function(element) {
            var bubbleCount = element.offsetWidth / 150 * 10;
            for (var i = 0; i <= bubbleCount; i++) {
                var size = Math.random() * (8 - 4) + 4;
                var top = Math.random() * (80 - 20) + 20;
                var left = Math.random() * (95 - 0) + 0;
                var delay = Math.random() * (3 - 0) + 0;
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const bubblesContainers = document.querySelectorAll(".background-bubbles__bubbles");
        const bubbleCount = 16;
        bubblesContainers.forEach((bubblesContainer => {
            if (bubblesContainer) for (let i = 0; i < bubbleCount; i++) {
                const bubble = document.createElement("div");
                bubble.classList.add("background-bubbles__bubble");
