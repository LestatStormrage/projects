(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            ...
            if (e.target.closest(".menu__link")) menuClose();
        }));
    }
    function menuClose() {
        bodyUnlock();
    ...
    const parallax = parallax_Parallax;
    ...
    function initSliders() {
        if (document.querySelector(".main-section__slider")) {
            new swiper_core_Swiper(".main-section__slider", {
            ...
            document.querySelectorAll(".category__slider").forEach(((sliderElement, index) => {
                const swiper2 = new swiper_core_Swiper(sliderElement, {
                ...
                swiper2Instances.push(swiper2);
                sliderElement.querySelectorAll(".category__slide").forEach((slide => {
                    slide.addEventListener("click", (function() {
                        toggleSlide(slide, swiper2);
                    }));
                }));
            }));
            function toggleSlide(slide, swiper) {
                ...
                const image = slide.querySelector(".slide-category__image");
                const container = slide.querySelector(".slide-category__container");
                slide.swiperInstance = swiper;
                const flsModules = {
                    parallax: null
                };
                if (isFullscreen) {
                    ...
                    resetParallaxStyles(image, container);
                    history.pushState({
                        fullscreen: true
                    }, "");
                } else {
                    ...
                    if (!flsModules.parallax) flsModules.parallax = new parallax(document.querySelectorAll("[data-prlx-parent]")); else flsModules.parallax.setEvents();
                    history.back();
                }
                swiper.update();
            }
            function resetParallaxStyles(image, container) {
            ...
            window.addEventListener("popstate", (function(event) {
                if (event.state && event.state.fullscreen) {
                    const fullscreenSlide = document.querySelector(".fullscreen-slide");
                    if (fullscreenSlide) {
                        const swiper = fullscreenSlide.swiperInstance;
                        fullscreenSlide.classList.remove("fullscreen-slide");
                        document.body.style.overflow = "";
                        if (swiper) {
                            swiper.allowSlidePrev = true;
                            swiper.allowSlideNext = true;
                            ...
                            const image = fullscreenSlide.querySelector(".slide-category__image");
                            const container = fullscreenSlide.querySelector(".slide-category__container");
                            image.setAttribute("data-coefficient", "4");
                            container.setAttribute("data-coefficient", "3");
                            const flsModules = {
                                parallax: null
    ...
    function handlePopup(openBtnId, popupContainerId, closeBtnId) {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        async function formSend(e) {
            ...
            } else alert("Bitte füllen Sie die Pflichtfelder aus");
        }
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const seriesButtons = document.querySelectorAll(".filter-catalogue__series .filter-catalogue__item");
        const categoryButtons = document.querySelectorAll(".filter-catalogue__categories .filter-catalogue__item");
        ...
        function applyFilters() {
            ...
            items.forEach((item => {
                ...
                const matchesSeries = activeSeries ? itemCategories.includes(activeSeries) : true;
                const matchesCategories = activeCategories.size > 0 ? [ ...activeCategories ].some((filter => itemCategories.includes(filter))) : true;
                if (anyActiveFilters && matchesSeries && matchesCategories) showItem(item); else if (!anyActiveFilters) showItem(item); else hideItem(item);
            }));
            toggleActiveClass(allButton, !anyActiveFilters);
            updateSeriesDisabledState();
        }
        function updateSeriesDisabledState() {
            seriesButtons.forEach((button => {
                const seriesFilter = button.getAttribute("data-filter");
                const hasMatch = Array.from(items).some((item => {
                    const itemCategories = item.getAttribute("data-category").split(",").map((cat => cat.trim()));
                    return itemCategories.includes(seriesFilter) && [ ...activeCategories ].some((cat => itemCategories.includes(cat)));
                }));
                button.classList.toggle("filter-disabled", !hasMatch && activeCategories.size > 0);
            }));
        }
        ...
        function scrollToCatalogue() {
            if (window.innerWidth > 768) catalogueSection.scrollIntoView({
                behavior: "smooth",
                block: "start"
            });
        }
        seriesButtons.forEach((button => {
            button.addEventListener("click", (() => {
                activeSeries = activeSeries === button.getAttribute("data-filter") ? null : button.getAttribute("data-filter");
                seriesButtons.forEach((btn => toggleActiveClass(btn, btn === button && activeSeries)));
                toggleActiveClass(allButton, false);
                applyFilters();
                scrollToCatalogue();
            }));
        }));
        categoryButtons.forEach((button => {
            button.addEventListener("click", (() => {
                ...
                if (activeCategories.has(filter)) {
                    ...
                    toggleActiveClass(button, false);
                } else {
                    activeCategories.add(filter);
                    toggleActiveClass(button, true);
                }
                toggleActiveClass(allButton, false);
                applyFilters();
                scrollToCatalogue();
        ...
        allButton.addEventListener("click", (() => {
            ...
            toggleActiveClass(allButton, true);
            applyFilters();
            scrollToCatalogue();
        }));
