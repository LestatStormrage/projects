(() => {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const filterButtons = document.querySelectorAll(".filter-catalogue__item");
        const items = document.querySelectorAll(".catalogue__item");
        let activeFilters = new Set;
        filterButtons.forEach((button => {
            button.addEventListener("click", (() => {
                const filter = button.getAttribute("data-filter");
                if (filter === "all") activeFilters.clear(); else if (activeFilters.has(filter)) activeFilters.delete(filter); else activeFilters.add(filter);
        ...
        function applyFilters() {
            items.forEach((item => {
                ...
                const matchesFilter = [ ...activeFilters ].some((filter => itemCategories.includes(filter)));
                if (activeFilters.size === 0 || matchesFilter) showItem(item); else hideItem(item);
            }));
        }
